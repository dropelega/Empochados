...
void loop()
{
  ...
  switch (state) {
    ...
    case 2://vision artificial
      //hablar con python
      lcd.setCursor ( 0, 0 );
      lcd.print("Coloque la baraja   ");
      lcd.setCursor ( 0, 1 );
      lcd.print("en su zona          ");
      lcd.setCursor ( 0, 2);
      lcd.print("y pulse el boton OK ");
      lcd.setCursor ( 0, 3 );
      lcd.print("                    ");
      if (okButtonState == HIGH) {
        if (enterok == true ) {
    ...
    case 3://apuestas
      ...
      if (plusButtonState == HIGH) {
        if (enterplus == true && bets[better - 1] < inhand) {
          bets[better - 1]++;
          /*if(better==crupier){//meter condiciones de maxima y minima apuesta
            summer=0;
            for(int i=0;i<6;i++){
      ...
      if (lessButtonState == HIGH) {
        if (enterless == true && bets[better - 1] > 0) {
          bets[better - 1]--;
          if (better == crupier) {
            int summer = 0;
            for (int i = 0; i < 6; i++) {
      ...
      if (okButtonState == HIGH) {
        if (enterok == true) {
          if (bazer == bets[better - 1]) {
            if(beer=="Oros"){
              score[better - 1] += (10 + bazer * 5)*2;
            }else{
              score[better - 1] += 10 + bazer * 5;
            }
          } else {
            if(beer=="Oros"){
              score[better - 1] -= (abs(bazer - bets[better - 1]) * 5)*2;
            }else{
